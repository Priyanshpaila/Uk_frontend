...
function extractHeroPieces(raw?: string) {
  if (!raw) return { title: null as string | null, intro: null as string | null };
  ...
  const intro = pMatch ? pMatch[1].trim() : null;
...
function Row({
  ...
  const handleClick = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (token) {
        try {
          const base = process.env.NEXT_PUBLIC_ADMIN_BASE_URL ?? 'http://127.0.0.1:8001'
      ...
      if (item.id === 'existing') {
        // Destination for existing patients after auth
        const destination = '/private-services/weight-loss/reorder';

        if (loggedIn) {
          router.push(destination);
        } else {
          // Pass intended destination to the auth page and also persist to localStorage as a fallback
          try { localStorage.setItem('post_login_redirect', destination); } catch {}
          router.push(`/auth?type=existing&next=${encodeURIComponent(destination)}`);
        }
      } else {
...
function TopSteps({
  ...
  page?: { title?: string; meta_title?: string | null };
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 items-start">
          <div className="md:col-span-6">
            <h1 className="text-4xl font-semibold leading-tight text-gray-900">
              {heroTitle ?? page?.meta_title ?? 'What do the next steps look like for your weight loss journey'}
            </h1>
            <p className="mt-4 text-gray-600">
              {heroIntro ?? 'Before you begin please choose the right path. This helps us gather the information we need to review your case quickly and safely.'}
            </p>
...
export default function WeightLossTemplate({
  ...
  page?: { title?: string; meta_title?: string | null; content?: string | null };
}) {
  const { title: heroTitle, intro: heroIntro } = extractHeroPieces(page?.content ?? undefined);

  return (
    <div className="bg-gray-50">
      <TopSteps page={page} heroTitle={heroTitle} heroIntro={heroIntro} />
