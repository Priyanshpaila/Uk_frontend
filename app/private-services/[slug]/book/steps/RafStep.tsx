 ...
 *
 * Fetches a question schema for the current service slug and renders a simple dynamic form.
 * Answers are kept in localStorage (so users don't lose progress on refresh) and can be
 * POSTed back to the API. This is intentionally minimal and can be styled/extended later.
 *
 * Expected API (adjust endpoints if different):
 *  - GET  `${NEXT_PUBLIC_API_BASE}/api/services/{slug}/forms` -> { service: {...}, raf_form: { id, name, form_type, schema } }
 *  - POST `${NEXT_PUBLIC_API_BASE}/api/services/{slug}/raf/answers` with { answers } body
 */
...
function getFirstSearchParamNumber(search: any, names: string[]): number | undefined {
  try {
...
function toQuestionArray(input: any): Question[] {
  ...
  if (Array.isArray(input) && input.every((sec) => Array.isArray((sec as any)?.fields))) {
    ...
    for (const sec of input as any[]) {
      const secTitle = String((sec as any)?.label ?? (sec as any)?.title ?? (sec as any)?.name ?? 'Section');
      const secKey = String(
        (sec as any)?.key ??
        (sec as any)?.data?.key ??
        slugify(secTitle)
      ...
      for (const f of (sec.fields || [])) {
        ...
        const mappedType: QuestionType =
          rawType === 'radio' ? 'select' :
          rawType === 'multiselect' || rawType === 'multi_select' || rawType === 'checkboxes' ? 'multiselect' :
          rawType === 'textarea' || rawType === 'text_area' ? 'textarea' :
          rawType === 'date' || rawType === 'datepicker' ? 'date' :
          rawType === 'number' || rawType === 'numeric' ? 'number' :
          rawType === 'boolean' || rawType === 'yesno' ? 'boolean' :
          rawType === 'file' || rawType === 'file_upload' ? 'file' :
          'text';
        ...
        const opts = Array.isArray(f.options)
          ? f.options.map((o: any) =>
              typeof o === 'string'
                ? { value: o, label: o }
                : { value: String(o.value ?? o.id ?? o), label: String(o.label ?? o.name ?? o) }
            )
        ...
        out.push({
          ...
          label: String(f.label ?? f.title ?? f.name ?? f.data?.label ?? 'Question'),
          ...
          accept: String(f.accept ?? f.data?.accept ?? 'image/*,application/pdf'),
          sectionKey: secKey,
  ...
  if (Array.isArray(input)) {
    ...
    input.forEach((x: any, i: number) => {
      ...
      const mappedType: QuestionType =
        ...
        rawType === 'date' || rawType === 'datepicker' ? 'date' :
        rawType === 'multiselect' || rawType === 'multi_select' || rawType === 'checkboxes' ? 'multiselect' :
        rawType === 'select' || rawType === 'dropdown' || rawType === 'radio' ? 'select' :
        rawType === 'file' || rawType === 'file_upload' ? 'file' :
        'text';
      ...
      const label = x.label ?? x.title ?? x.name ?? x.data?.label ?? `Question ${i + 1}`;
      ...
      const sTitle = String(x.sectionTitle ?? x.data?.sectionTitle ?? curSectionTitle ?? (sKey ? 'Section' : ''));
  ...
  if (typeof input === 'object') {
    const maybe = (input.schema ?? input.questions ?? input.fields ?? input.form?.schema);
...
export default function RafStep() {
  ...
  const [sessionId, setSessionId] = useState<number | undefined>(() => resolveInitialSessionId(search as any))
  ...
  useEffect(() => {
    ...
    try {
      const fromUrl = getFirstSearchParamNumber(search as any, ['session_id'])
      const cookies = {
        consultation_session_id: getCookie('consultation_session_id'),
        pe_consultation_session_id: getCookie('pe_consultation_session_id'),
        pe_dot_consultation_session_id: getCookie('pe.consultation_session_id'),
        csid: getCookie('csid'),
      } as const
      const storage = {
        ls_consultation_session_id: typeof window !== 'undefined' ? localStorage.getItem('consultation_session_id') : null,
        ls_pe_consultation_session_id: typeof window !== 'undefined' ? localStorage.getItem('pe_consultation_session_id') : null,
        ls_consultationSessionId: typeof window !== 'undefined' ? localStorage.getItem('consultationSessionId') : null,
        ss_consultation_session_id: typeof window !== 'undefined' ? sessionStorage.getItem('consultation_session_id') : null,
        ss_pe_consultation_session_id: typeof window !== 'undefined' ? sessionStorage.getItem('pe_consultation_session_id') : null,
        ss_consultationSessionId: typeof window !== 'undefined' ? sessionStorage.getItem('consultationSessionId') : null,
      } as const
      console.log('[raf] session snapshot', { sessionId, fromUrl, cookies, storage })
    } catch {}
  }, [sessionId, search])
    ...
    async function run() {
      ...
      try {
        const res = await fetch(
          `${apiBase}/api/services/${encodeURIComponent(slug)}/forms`,
          { credentials: 'omit', headers: { ...getAuthHeaders(), Accept: 'application/json' } }
        );
        ...
        // Accept raf_form.schema or full raf_form or other common keys
        const rawSchema = payload?.raf_form?.schema ?? payload?.schema ?? payload?.questions ?? payload?.raf_form ?? null;
    ...
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent('raf:updated', { detail: { slug, sessionId, count: Object.keys(answers || {}).length } })
        );
  ...
  const normalize = (v: any) => {
    ...
    if (typeof v === 'object' && 'value' in v) return normalize((v as any).value);
    return v;
  };
  ...
  const isVisible = (q: Question): boolean => {
    ...
    if (c.equals !== undefined) {
      ...
      if (eq === 'yes') return v === true || v === 'yes' || v === 'y' || v === 'true';
      if (eq === 'no') return v === false || v === 'no' || v === 'n' || v === 'false';
      return v === eq;
    }
    if (c.notEquals !== undefined) return normalize(val) !== normalize(c.notEquals);
    return !!val;
  };
  ...
  const sections = useMemo(() => {
    ...
    for (const q of visibleQuestions) {
      ...
      if (!meta.has(key)) {
        meta.set(key, { title: q.sectionTitle ?? (key === SECTION_NONE ? 'General' : 'Section') });
        order.push(key);
      }
    }
    return { order, meta };
  ...
  const currentSectionTitle =
    sections.meta.get(currentSectionKey)?.title ?? (currentSectionKey === SECTION_NONE ? 'General' : 'Section');

  const questionsInSection = useMemo(
    () => visibleQuestions.filter((q) => (q.sectionKey ?? SECTION_NONE) === currentSectionKey),
    [visibleQuestions, currentSectionKey]
  );

  const requiredUnansweredInSection = useMemo(
    () => questionsInSection.filter((q) => q.required && (answers[q.id] === undefined || answers[q.id] === null || answers[q.id] === "" || (Array.isArray(answers[q.id]) && answers[q.id].length === 0))),
  ...
  const requiredUnanswered = useMemo(
    () => visibleQuestions.filter((q) => q.required && isEmptyAnswer(answers[q.id])),
  ...
  const remainingRequired = useMemo(() => requiredUnanswered.length, [requiredUnanswered]);
  const answeredRequired = useMemo(() => Math.max(totalRequired - remainingRequired, 0), [totalRequired, remainingRequired]);
  const percentComplete = useMemo(() => totalRequired ? Math.round((answeredRequired / totalRequired) * 100) : 100, [answeredRequired, totalRequired]);
  ...
  const onBack = () => {
    router.push(`/private-services/${encodeURIComponent(slug)}/book?step=${encodeURIComponent(prevStep)}`);
  };

  type UploadedFile = { name: string; size?: number; type?: string; path?: string; url?: string };
  ...
  // Reuses the working /api/uploads/intake-image endpoint from weight-loss intake
  // Sends multipart/form-data with Bearer only, no cookies
  async function uploadIntakeImage(
    apiBase: string,
    file: File,
    kind: string = 'raf'
  ): Promise<{ ok: boolean; url?: string; path?: string; message?: string }> {
    try {
  ...
  function normalizeUploadResponse(json: any): UploadedFile[] {
    ...
    if (json?.path || json?.url || json?.name) return [json as UploadedFile];
    return [];
  }

  async function uploadFilesForQuestion(qid: string, files: File[]): Promise<UploadedFile[]> {
    // We now reuse the proven /api/uploads/intake-image endpoint.
    // Each file is uploaded individually using Bearer auth only (no cookies, no CSRF).
    // We tag uploads with kind 'raf' so the backend can file them under travel/raf evidence.
    ...
    for (const f of files) {
      ...
      if (res.ok) {
        ...
        // if one file fails, we just skip it so user can still proceed;
        // future UX could surface res.message
        if (search.get('debug') === '1') {
          try {
            console.warn('[raf] upload failed', { qid, file: f.name, message: res.message });
          } catch {}
        }
      }
    }
  ...
  async function postAnswersToConsultation(override?: Answers) {
    const sid = sessionId != null ? Number(sessionId) : NaN
    ...
    try {
      ...
      if (xsrf) {
        try { headers['X-XSRF-TOKEN'] = decodeURIComponent(xsrf) } catch { headers['X-XSRF-TOKEN'] = xsrf }
      }
      ...
      if (search.get('debug') === '1') {
        try { console.log('[raf] posting consultation answers', { sid, payload, api: `${apiBase}/api/consultations/${encodeURIComponent(String(sid))}/answers` }) } catch {}
      }

      const res = await fetch(
        `${apiBase}/api/consultations/${encodeURIComponent(String(sid))}/answers`,
        {
          method: 'POST',
          headers,
          credentials: 'include',
      ...
      if (!res.ok) {
        ...
        console.warn('[raf] consult answers failed', res.status, json || text || '(empty)')
  ...
  const onContinue = async () => {
    ...
    for (const [qid, items] of Object.entries(uploadedByQ)) {
      // If we uploaded, replace the answer value with the server metadata objects
    ...
    try {
      try {
        localStorage.setItem('last_raf', JSON.stringify({ slug, sessionId, answers, ts: Date.now() }))
      } catch {}

      // Add: sessionStorage snapshot and raf:submit event with debug log
      try {
        sessionStorage.setItem('raf.last_submit', JSON.stringify({ slug, sessionId, answers, ts: Date.now() }))
      } catch {}

      try {
        window.dispatchEvent(new CustomEvent('raf:submit', { detail: { slug, sessionId, answers } }))
        if (search.get('debug') === '1') {
      ...
      // legacy best effort
      try {
        // make sure CSRF cookie exists for legacy endpoints that use web middleware
        await ensureCsrfCookie(apiBase);
        const xsrf = getCookie('XSRF-TOKEN');
        const postUrls = [
          `${apiBase}/api/services/${encodeURIComponent(slug)}/raf/answers`,
          `${apiBase}/api/services/${encodeURIComponent(slug)}/forms/answers`,
        ]
        for (const url of postUrls) {
          try {
            ...
            if (xsrf) {
              try {
                legacyHeaders['X-XSRF-TOKEN'] = decodeURIComponent(xsrf);
              } catch {
            ...
            const r = await fetch(url, {
              ...
              credentials: 'include',
              body: JSON.stringify({ answers: answersToSend, session_id: sessionId, service: slug, slug }),
            });

            if (!r.ok && r.status === 419) {
              // try once more after forcing a fresh CSRF cookie
              if (typeof window !== 'undefined' && (window as any).__raf_legacy_retry__ !== url) {
                (window as any).__raf_legacy_retry__ = url;
                await ensureCsrfCookie(apiBase);
                const retryXsrf = getCookie('XSRF-TOKEN');
                if (retryXsrf) {
                  try {
                    legacyHeaders['X-XSRF-TOKEN'] = decodeURIComponent(retryXsrf);
                  } catch {
      ...
      if (sessionId) {
        ...
        try { document.cookie = `pe_consultation_session_id=${sessionId}; path=/; max-age=604800` } catch {}
      }
    ...
    <>
      {showProgressBar && (
        <div className="sticky top-0 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur border-b border-gray-200 dark:border-slate-700">
          <div className="max-w-4xl mx-auto px-4 py-2 flex items-center gap-4">
            <div className="flex-1">
              <div className="flex items-center justify-between text-sm">
                <span className="font-medium text-gray-700 dark:text-slate-100">Progress</span>
                <span className="rounded-full px-2 py-0.5 text-xs bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200">{remainingRequired} left</span>
              </div>
              <div
                ...
                aria-valuemax={100}
              >
                <div className="h-2 rounded-full bg-emerald-600" style={{ width: `${percentComplete}%` }} />
              </div>
            </div>
          ...
          {saveFlash && <div className="text-center text-xs text-gray-600 dark:text-slate-300 pb-2">{saveFlash}</div>}
        </div>
      )}
      <div className="max-w-4xl mx-auto mt-4">
      <h2 className="text-2xl md:text-3xl font-semibold mb-2">Risk Assessment</h2>
      {search.get('debug') === '1' && (
        <div className="mb-2 text-xs text-gray-600 dark:text-slate-300">session {sessionId ?? 'none'}</div>
      )}
      <p className="text-gray-600 mb-6">
        Please answer a few questions to help our clinicians assess your suitability for this treatment.
      </p>
      ...
      {!loading && !error && questions.length === 0 && (
        <div className="rounded-xl border p-6 bg-white shadow-sm text-gray-600">
          No questions for this service yet. You can still continue to the next step.
        </div>
      )}
        ...
        <>
          ...
          <div className="mb-4 flex items-center justify-between text-sm">
            <div className="text-gray-700 dark:text-slate-200">
              Section {sectionIdx + 1} of {sections.order.length} • {currentSectionTitle}
            </div>
            {sections.order.length > 1 && (
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() => setSectionIdx((i) => Math.min(sections.order.length - 1, i + 1))}
      ...
      <div className="mt-8 flex items-center justify-between gap-3">
        <button
          ...
          onClick={() => setSectionIdx((i) => Math.max(0, i - 1))}
          disabled={sectionIdx === 0}
          className="px-5 py-2 rounded-full border border-gray-300 text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-40"
        >
          Previous Section
        </button>
        ...
        <div className="text-sm text-gray-500">
          {requiredUnansweredInSection.length > 0 && (
            <div>{requiredUnansweredInSection.length} required {requiredUnansweredInSection.length === 1 ? "question" : "questions"} left in this section</div>
          )}
          {requiredUnanswered.length > 0 && (
            <div>{requiredUnanswered.length} required {requiredUnanswered.length === 1 ? "question" : "questions"} left in total</div>
          )}
        </div>
        ...
        <button
          ...
          onClick={() => {
            if (sectionIdx < sections.order.length - 1) {
              setSectionIdx((i) => Math.min(sections.order.length - 1, i + 1));
            } else {
              onContinue();
            }
          }}
          disabled={submitting || (sectionIdx >= sections.order.length - 1 && requiredUnanswered.length > 0)}
          className={`px-6 py-2 rounded-full text-white ${submitting || (sectionIdx >= sections.order.length - 1 && requiredUnanswered.length > 0) ? "bg-emerald-300" : "bg-emerald-600 hover:bg-emerald-700"}`}
        >
          {submitting ? "Saving…" : (sectionIdx < sections.order.length - 1 ? "Next Section" : "Continue")}
        </button>
      </div>
    </div>
    </>
...
function QuestionField({
  ...
  const base = "block w-full rounded-xl border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500";
  return (
    <div id={`q-${q.id}`} className="rounded-xl border p-4 bg-white shadow-sm">
      <label className="block font-medium mb-1">
        {q.label} {q.required && <span className="text-red-600">*</span>}
      </label>
      {q.helpText && <p className="text-sm text-gray-500 mb-2">{q.helpText}</p>}
      ...
      {q.type === "multiselect" && (
        <div className="grid sm:grid-cols-2 gap-2">
          {(q.options || []).map((opt) => {
            ...
            return (
              <label key={opt.value} className="flex items-center gap-2 rounded-lg border border-gray-300 px-3 py-2">
                <input
                  type="checkbox"
                  checked={checked}
                  onChange={(e) => {
        ...
        <div className="space-y-2">
          <input
            ...
            onChange={(e) => {
              ...
              setFileStash((prev) => ({ ...prev, [q.id]: kept }));
              // Store lightweight meta to keep UI responsive and localStorage serialisable
              const meta = kept.map(f => ({ name: f.name, size: f.size, type: f.type || 'file' }));
              onChange(meta);
            }}
          />
          {Array.isArray(value) && value.length > 0 && (
            <ul className="text-sm text-gray-600">
              {value.map((f: any, i: number) => (
                <li key={i}>
                  {f.name} {typeof f.size === 'number' ? `(${Math.round(f.size / 1024)} KB)` : ''}
                </li>
              ))}
            </ul>
          )}
          {fileStash[q.id]?.length ? (
            <div className="text-xs text-gray-500">files queued and will upload on continue</div>
          ) : null}
          {!q.helpText && (
            <p className="text-xs text-gray-500">Max 10MB. PDF or image.</p>
          )}
        </div>
      )}
    </div>
